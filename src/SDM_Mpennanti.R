...
rawdata<- gbif(genus = 'Martes', species = 'pennanti') 
rawdata[,c('lat','lon')] 
na.omit(rawdata[,c('lat','lon')])
Mpennanti <- na.omit(rawdata[,c('lat','lon')])
...
plot(c(-96.62, -83.32), c(45.602, 50.49), mar=par("mar"), xlab="longitude", ylab="latitude", xaxt="n", yaxt="n",
type="n", main="M.pennanti Presence Data")
rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4], col="lightcyan")
map("state", xlim=c(-96.62, -83.32), ylim=c(45.602, 50.49), fill=T, col="honeydew", add=T)

#text(x=lon, y=lat, "state name", col="black", cex=.3) 

# plot the points
points(Mpennanti$lon, Mpennanti$lat, col="darkolivegreen4", pch=20, cex=0.5)
axis(1,las=1)
axis(2,las=1)
box()

# create sequences of latitude and longitude values to define the grid
longrid = seq(-96.62, -83.32,0.05)
latgrid = seq(45.602, 50.49,0.05)

# identify points within each grid cell, draw one at random
subs = c()
for(i in 1:(length(longrid)-1)){
  for(j in 1:(length(latgrid)-1)){
    gridsq = subset(Mpennanti, lat > latgrid[j] & lat < latgrid[j+1] & lon > longrid[i] & lon < longrid[i+1])    
    if(dim(gridsq)[1]>0){
...
#crop data
YbrevRange_Mpennanti = extent(-96.62, -83.32,45.5, 50.3) 
BClim = crop(BClim, YbrevRange_Mpennanti)
writeRaster(BClim, filename="/Users/annacalderon/Desktop/gENM/data/wc2-5/", overwrite=T)
...
map("state", xlim=c(-96.62,-83.32), ylim=c(45.6026, 50.49), fill=F, col="honeydew", add=T)
...
map("state", xlim=c(-96.62,-83.32), ylim=c(45.602,50.49), fill=F, col="black", add=T)
points(bg,col="snow",pch=1,cex=0.2)
points(Mpennanti$lon, Mpennanti$lat, col="darkgreen", pch=20, cex=0.5)
